...
# Slack Webhook URL
SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/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'  # Replace with your Slack webhook URL
...
def send_slack_message(message):
    ...
    if response.status_code != 200:
        st.error(f"Failed to send Slack message: {response.text}")
...
def update_inventory(product, new_stock):
    if product in inventory:
        ...
        if current_stock < minimum_stock:
            alert_message = f"ALERT: Current stock ({current_stock} kg) is below the minimum threshold ({minimum_stock} kg). Restock immediately!"
            st.warning(alert_message)
            send_slack_message(alert_message)
        elif current_stock > capacity:
            alert_message = f"WARNING: Current stock ({current_stock} kg) exceeds capacity ({capacity} kg). Consider reducing stock."
            st.warning(alert_message)
            send_slack_message(alert_message)
        else:
...
def log_damage(product, damaged_quantity):
    if product in inventory:
        ...
        if inventory[product]["current_stock"] < inventory[product]["minimum_stock"]:
            alert_message = f"ALERT: Current stock is below the minimum threshold ({inventory[product]['minimum_stock']} kg). Restock immediately!"
            st.warning(alert_message)
            send_slack_message(alert_message)
        elif damaged_quantity > 100:  # Example threshold for high damage
            alert_message = f"WARNING: High damage recorded for {product}! Investigate causes."
            st.warning(alert_message)
            send_slack_message(alert_message)
    else:
        st.error("Error: Product not found.")
...
def log_transport_delay(product, delay_days):
    if product in inventory:
        ...
        alert_message = f"Transport delay logged for {product}. Delay duration: {delay_days} days."
        st.warning(alert_message)
        send_slack_message(alert_message)
    else:
        st.error("Error: Product not found.")
...
def view_inventory():
    ...
    # Visualization: Bar chart for inventory status
    ...
    ax.bar(x, capacities, width=0.4, label="Capacity", color='green', alpha=0.5)
...
if options == "Predict Risk Factor":
    ...
    if st.button("Predict Risk Factor"):
        ...
        # Ensure risk_factor is a float
        risk_factor = (risk_factor)
        
        st.success(f"Predicted Risk Factor: **{risk_factor}**")
        if risk_factor >= "High":
            send_slack_message(f"WARNING: High risk predicted for sugar supply chain! Risk Factor.")
        elif risk_factor >="Low":
            send_slack_message(f"ALERT: Medium risk predicted for sugar supply chain. Risk Factor.")
